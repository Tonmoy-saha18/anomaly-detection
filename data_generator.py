...
def generate_date_time(start):
    time_stamp = start + timedelta(seconds=random.randint(0, 600))
    if time_stamp.time().hour == 22 and time_stamp.time().minute > 00:
        return time_stamp + timedelta(days=1) - timedelta(hours=14)
    return time_stamp
...
def getAllTypeOfAccountList(df) -> tuple:
    ...
    customer_accounts = pd.concat(
        [customer_info['from_internal_account'], customer_info['to_internal_account'], customer_info1['from_internal_account'],
    ...
    agent_accounts = pd.concat([agent_info1['to_internal_account'], agent_info2['from_internal_account']]).unique().tolist()
    ...
    bank_accounts = pd.concat([bank_info1['to_internal_account'], bank_info2['from_internal_account']]).unique().tolist()
...
def generateCustomerToCustomerTransaction(customer_account_list, account_to_balance_map) -> list[list[str]]:
    ...
    for _ in customer_account_list:
        ...
        for i in range(num_transactions):
            ...
            amount = random.uniform(*customer_amount_range)
            ...
            before_transaction_balance = account_to_balance_map[from_account]
            ...
            transaction_list.append([transaction_datetime, amount, fee_amount, receiver_credit_amount,
                                     ...
                                     from_account, to_account, transaction_type[0], before_transaction_balance])
...
def generateCustomerToAgentTransaction(customer_account_list, agent_account_list, account_to_balance_map) -> list[list[str]]:
    ...
    for _ in customer_account_list:
        ...
        for i in range(num_transactions):
            ...
            transaction_list.append([transaction_datetime, amount, fee_amount, receiver_credit_amount,
                                     sender_debit_amount, depositor_running_balance, withdrawer_running_balance,
                                     from_account, to_account, transaction_type[1], before_transaction_balance])
    return transaction_list


def generateAgentToCustomerTransaction(customer_account_list, agent_account_list, account_to_balance_map) -> list[list[str]]:
    global start_time
    global count
    ...
    for _ in agent_account_list:
        ...
        for i in range(num_transactions):
            ...
            to_account = customer_account_list[random.randint(0, len(customer_account_list) - 1)]
            amount = random.uniform(*agent_amount_range)
            fee_amount = 0
            sender_debit_amount = amount + fee_amount
            if account_to_balance_map[from_account] < sender_debit_amount:
                count += 1
            ...
            transaction_list.append([transaction_datetime, amount, fee_amount, receiver_credit_amount,
                                     sender_debit_amount, depositor_running_balance, withdrawer_running_balance,
                                     from_account, to_account, transaction_type[2], before_transaction_balance])

    return transaction_list


def generateCustomerToMerchantTransaction(customer_account_list, merchant_account_list, account_to_balance_map) -> list[list[str]]:
    global start_time
    global count
    ...
    for _ in customer_account_list:
        ...
        for i in range(num_transactions):
            ...
            amount = random.uniform(*merchant_amount_range)
            ...
            transaction_list.append([transaction_datetime, amount, fee_amount, receiver_credit_amount,
                                     sender_debit_amount, depositor_running_balance, withdrawer_running_balance,
                                     from_account, to_account, transaction_type[3], before_transaction_balance])
    return transaction_list


def generateCustomerToBankTransaction(customer_account_list, bank_account_list, account_to_balance_map) -> list[list[str]]:
    global start_time
    global count
    ...
    for _ in customer_account_list:
        ...
        for i in range(num_transactions):
            ...
            transaction_list.append([transaction_datetime, amount, fee_amount, receiver_credit_amount,
                                     sender_debit_amount, depositor_running_balance, withdrawer_running_balance,
                                     from_account, to_account, transaction_type[4], before_transaction_balance])
...
def generateBankToCustomerTransaction(customer_account_list, bank_account_list, account_to_balance_map) -> list[list[str]]:
    ...
    for _ in bank_account_list:
        ...
        for i in range(num_transactions):
            ...
            transaction_list.append([transaction_datetime, amount, fee_amount, receiver_credit_amount,
                                     sender_debit_amount, depositor_running_balance, withdrawer_running_balance,
                                     from_account, to_account, transaction_type[5], before_transaction_balance])

    return transaction_list


def generateCardToCustomerTransaction(customer_account_list, card_number_list, account_to_balance_map) -> list[list[str]]:
    global start_time
    global count
    ...
    for _ in card_number_list:
        ...
        for i in range(num_transactions):
            ...
            transaction_list.append([transaction_datetime, amount, fee_amount, receiver_credit_amount,
                                     sender_debit_amount, depositor_running_balance, withdrawer_running_balance,
                                     from_account, to_account, transaction_type[6], before_transaction_balance])
...
def dataFrameToCsv(df) -> None:
    ...
    df.to_csv("transaction_report_2020_2024_v2.csv", index=False)
...
if __name__ == "__main__":
    ...
    columns = ['transaction_date_time', 'amount', 'fee_amount', 'receiver_credit_amount',
               'sender_debit_amount', 'depositor_running_balance', 'withdrawer_running_balance',
               'from_account', 'to_account', 'transaction_type', 'before_transaction_balance']
    ...
    for i in range(epoch):
        ...
        card_to_customer_transaction_list = generateCardToCustomerTransaction(customer_account_list=cA, card_number_list=cN,
                                                                              account_to_balance_map=accountToBalanceMap)
